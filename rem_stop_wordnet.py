#### OBTAIN SIMILARITY VALUE B/W filtered the title and description(of the videos obtained from a search keyword) AND TRAINING SET

#### Channel ids of videos with greater value than the THRESHOLDare obtained (hard coded -----later on take threshold as the average of the similarity values )


### filteration=== here, stop words are only removed(((urls NOT REMOVED YET)))
### NOT STORED IN MONGO DB NOR IN JSON FORMAT
...
### filter the title and description of the videos by removing the STOP WORDS(extended the provided set in nltk)
    ...
    # print(word_tokens)
    # print(filtered_sentence)####### IMPORTANT CODE LINE
    # record=db.collection.insert(channels)
     
#  print (channels) #######important line to print title and description and channel id

     list1 = ['rape', 'man','hot','sex']
     list2 = filtered_sentence
     list = []
...
if __name__ == "__main__":
  argparser.add_argument("--q", help="Search term", default="women harrasment office")
  argparser.add_argument("--max-results", help="Max results", default=7) ###### OUTPUT= 11 (I.E. 1 less than the default value set) happens in every code 
